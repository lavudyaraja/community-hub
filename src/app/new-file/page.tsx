...
function ValidationHubContent() {
  ...
  const [dataFiles, setDataFiles] = useState<DataFile[]>([
    ...
    {
      id: '2',
      name: 'audio_signal_beta.wav',
      type: 'audio',
      status: 'anomaly',
      anomalyType: 'Freq Spike',
      thumbnail: ''
    },
    {
      id: '3',
      name: 'thermal_pass_009.mp4',
      type: 'video',
      status: 'queued',
      duration: '04:22',
      codec: 'H.265',
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-['Space_Grotesk',sans-serif]">
      {/* Header */}
      <header className="flex items-center justify-between border-b border-slate-700 bg-slate-900/80 backdrop-blur-md px-10 py-3 sticky top-0 z-50">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-4 text-primary">
            ...
            <h2 className="text-white text-lg font-bold leading-tight tracking-tight">Data Intelligence Platform</h2>
          </div>
        </div>
        <div className="flex items-center gap-6">
          ...
          <div className="h-9 w-9 rounded-full bg-gradient-to-br from-primary to-amber-500 border-2 border-white/10" />
        </div>
      </header>
      ...
      <main className="max-w-[1600px] mx-auto px-10 py-8">
        ...
        <div className="grid grid-cols-12 gap-8">
          ...
          <div className="col-span-9 flex flex-col gap-6">
            ...
            {showValidationMode && submissionData ? (
              <section className="bg-slate-800 border border-slate-700 rounded-xl p-6">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-3">
                    <Shield className="w-6 h-6 text-amber-500" />
                    <h2 className="text-white text-xl font-bold">Select Validation Mode</h2>
                  </div>
                  <Badge variant="outline" className="bg-blue-500/10 text-blue-400 border-blue-500/20 text-[10px] font-bold uppercase tracking-widest">
                    Submission: {submissionData.file_name || submissionData.fileName || 'Unknown'}
                  </Badge>
                </div>

                <div className="mb-4 p-4 bg-slate-900/50 rounded-lg border border-slate-700">
                  <p className="text-slate-300 text-sm mb-2">
                    <span className="font-semibold">File:</span> {submissionData.file_name || submissionData.fileName || 'Unknown'}
                  </p>
                  <p className="text-slate-400 text-xs">
                    <span className="font-semibold">Type:</span> {submissionData.file_type || submissionData.fileType || 'Unknown'} • 
                    <span className="font-semibold ml-2">Size:</span> {submissionData.file_size || submissionData.fileSize || 0} bytes
                  </p>
                </div>
                ...
                <div className="flex gap-4">
                  <label className="flex-1 cursor-pointer group">
                    ...
                    />
                    <div className="flex flex-col gap-3 p-5 rounded-xl border-2 border-slate-700 bg-slate-900 peer-checked:border-primary peer-checked:bg-primary/10 transition-all hover:border-slate-500 disabled:opacity-50">
                      ...
                      <div>
                        ...
                        <p className="text-slate-400 text-sm">AI-driven validation will automatically approve this submission.</p>
                      </div>
                    </div>
                  </label>
                  ...
                  <label className="flex-1 cursor-pointer group">
                    <input 
                      ...
                      disabled={isValidatingSubmission}
                    />
                    <div className="flex flex-col gap-3 p-5 rounded-xl border-2 border-slate-700 bg-slate-900 peer-checked:border-amber-500 peer-checked:bg-amber-500/10 transition-all hover:border-slate-500 disabled:opacity-50">
                      <div className="flex justify-between items-start">
                      ...
                      <div>
                        ...
                        <p className="text-slate-400 text-sm">Review the file in the sandbox before validating.</p>
                      </div>
                    </div>
                  </label>
                </div>
                ...
                <div className="flex gap-3 mt-6">
                  <Button
                    className="flex-1"
                    onClick={() => handleValidationModeSelect(validationStrategy)}
                    disabled={isValidatingSubmission}
                  >
                    {isValidatingSubmission ? (
                      ...
                      <>
                        ...
                        Proceed with {validationStrategy === 'automated' ? 'Automated' : 'Manual'} Validation
                      </>
                    )}
                  </Button>
                  <Button
              ...
              <div className="flex gap-4">
                <label className="flex-1 cursor-pointer group">
                  <input 
                    ...
                    className="hidden peer"
                  />
                  <div className="flex flex-col gap-3 p-5 rounded-xl border-2 border-slate-700 bg-slate-900 peer-checked:border-primary peer-checked:bg-primary/10 transition-all hover:border-slate-500">
                    <div className="flex justify-between items-start">
                      <div className="p-2 bg-primary/20 rounded-lg text-primary">
                        <Brain className="w-6 h-6" />
                      </div>
                ...
                <label className="flex-1 cursor-pointer group">
                  <input 
                    ...
                    className="hidden peer"
                  />
                  <div className="flex flex-col gap-3 p-5 rounded-xl border-2 border-slate-700 bg-slate-900 peer-checked:border-amber-500 peer-checked:bg-amber-500/10 transition-all hover:border-slate-500">
                    <div className="flex justify-between items-start">
                      <div className="p-2 bg-amber-500/20 rounded-lg text-amber-500">
                        <UserSearch className="w-6 h-6" />
                      </div>
                    ...
                    <div className="p-4 bg-slate-800 border-t border-slate-700">
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            if (submissionId && file.id === submissionId) {
                        ...
                        >
                          {file.status === 'approved' ? 'APPROVED' : submissionId && file.id === submissionId ? 'VALIDATE' : 'APPROVE'}
                        </Button>
                        <Button
                          variant="destructive"
                          size="sm"
          ...
          <aside className="col-span-3 flex flex-col gap-6">
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 flex flex-col gap-6">
              ...
              <div className="space-y-4">
                <div className="flex flex-col gap-1.5">
                  ...
                  <Progress value={metrics.nodeStatus} className="h-1.5" />
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <Progress value={(metrics.latency / 300) * 100} className="h-1.5" />
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <Progress value={(metrics.anomalies / 20) * 100} className="h-1.5" />
                </div>
            ...
            <div className="bg-primary/5 border border-primary/20 rounded-xl p-6 flex flex-col gap-4">
              ...
              <div className="bg-slate-900/50 rounded-lg p-3 mb-2">
                ...
                <Progress value={(approvedCount / totalCount) * 100} className="h-1.5" />
              </div>
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-xl p-4">
              <div className="flex justify-between items-center mb-3">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Real-time Logs</span>
              ...
              <div className="font-mono text-[10px] space-y-2 overflow-hidden max-h-32">
                {logs.slice(-4).map((log, i) => (
                  <p key={i} className={log.type === 'warning' ? 'text-amber-500' : 'text-slate-500'}>
                    <span className={log.type === 'warning' ? 'text-amber-500' : 'text-primary'}>
                      [{log.time}]
                    </span> {log.message}
                  </p>
                ))}
          ...
          <div className="px-6 pb-6">
            ...
            <div className="flex gap-3">
              <Button
                ...
                onClick={() => {
                  if (selectedFile) {
                    if (submissionId && selectedFile.id === submissionId) {
                      handleManualValidation();
                    } else {
              ...
              >
                <CheckCircle2 className="w-4 h-4 mr-2" />
                {selectedFile?.status === 'approved' ? 'APPROVED' : submissionId && selectedFile?.id === submissionId ? 'VALIDATE FILE' : 'APPROVE FILE'}
              </Button>
              <Button
                variant="destructive"
                className="flex-1"
